...
// import { twitterSpaceTemplate, twitterVoiceHandlerTemplate } from './templates';
...
//   ): Promise<void> {
//     elizaLogger.log(
//       `[SttTtsPlugin] Streaming text to Janus: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`,
//     );
...
//                 elizaLogger.log(
//                   '[SttTtsPlugin] User interrupt detected during TTS playback',
//                 );
//                 isInterrupted = true;
//                 break;
//               }
...
//           } catch (error) {
//             elizaLogger.error(
//               '[SttTtsPlugin] Error sending audio frame:',
//               error,
//             );
//           }
//         }
...
//       } else {
//         elizaLogger.log(
//           '[SttTtsPlugin] Audio streaming completed successfully',
//         );
//       }
//     } catch (error) {
...
//       elizaLogger.log(
//         `[SttTtsPlugin] Starting new stream with ID: ${streamId}`,
//       );
...
//             elizaLogger.log(
//               `[SttTtsPlugin] Processing TTS chunk: "${textWithPauses.substring(0, 50)}${textWithPauses.length > 50 ? '...' : ''}"`,
//             );
...
//           } catch (error) {
//             elizaLogger.error(
//               '[SttTtsPlugin] Error processing TTS chunk:',
//               error,
//             );
//           } finally {
...
//             processTtsChunk();
//           } else if (
//             ttsBuffer.length >= minCharactersForProgressive &&
//             progressiveTimeout === null
//           ) {
...
//           if (
//             startStreamId &&
//             (!this.activeStreams.has(startStreamId) || signal.aborted)
//           ) {
//             elizaLogger.debug(
//               `[SttTtsPlugin] Ignoring stream-start from outdated stream`,
//             );
//             return;
//           }
...
//           'stream-chunk',
//           (chunk: string, chunkStreamId?: string) => {
//             // Ignore chunks if this stream is no longer active or has been aborted
//             if (
//               !chunkStreamId ||
//               !this.activeStreams.has(chunkStreamId) ||
//               signal.aborted
//             ) {
//               elizaLogger.debug(
//                 `[SttTtsPlugin] Ignoring chunk from outdated stream`,
//               );
//               return;
//             }
...
//           if (
//             !endStreamId ||
//             !this.activeStreams.has(endStreamId) ||
//             signal.aborted
//           ) {
//             elizaLogger.debug(
...
//       elizaLogger.error(
//         '[SttTtsPlugin] No current stream ID found or stream is no longer active',
//       );
//       return;
//     }
...
//       this.ttsAbortController?.signal.aborted
//     ) {
//       elizaLogger.log(
...
//     elizaLogger.log(
//       '[SttTtsPlugin] Starting Grok streaming response processing',
//     );
...
//             elizaLogger.log(
//               `[SttTtsPlugin] Detected action: ${detectedAction}`,
//             );
...
//           detectedAction = actionMatch[1];
//           elizaLogger.log(
//             `[SttTtsPlugin] Final detected action: ${detectedAction}`,
//           );
//         }
//       } else {
...
//     if (detectedAction) {
//       elizaLogger.log(
//         `[SttTtsPlugin] Response contained action: ${detectedAction}`,
//       );
